...
# metareg mod
metareg_mod <- readRDS("~/CnGV-CoGV-Meta-analysis/Data/model_output/mod_metareg_noyear_sp.RDS")

metareg_mod_BF <- bayesfactor_parameters(metareg_mod, null = c(log(1e-16),log(0.1))) #Effect sizes between 0 and 0.2 
...
metareg_modBF <-readRDS("~/CnGV-CoGV-Meta-analysis/Data/model_output/metareg_modBF.RDS")
metareg_modBF

effectsize::interpret_bf(exp(metareg_modBF$log_BF), include_value = T)

bayes_factor(int_mod, log = FALSE)
bayes_R2(metareg_mod, summary = FALSE)
q

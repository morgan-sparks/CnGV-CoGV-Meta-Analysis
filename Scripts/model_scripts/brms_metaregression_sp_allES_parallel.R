...
burn <- 7500
chains <- 4
cores <- 24 # this sets 4 cores per chain to run in parallel
...
mod_metareg_sp <- brm(
  log(abs(yi)) |  se(Standard.Deviation/sqrt(Sample.Size))  ~ alt_trait + Gradient + Class + # fixed effects
...
saveRDS(mod_metareg_sp, "/scratch/bell/sparks35/CNGV_analysis/output/mod_metareg_noyear_sp_wInt_allES_correct_parallel.RDS")
